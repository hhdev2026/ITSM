...
-- Ensure columns exist even if the table was created previously.
...
-- Example workflows: auto-assign by priority (Crítica/Alta) to least loaded agent.
...
from (
  values
    ...
    (
      '11111111-1111-1111-1111-111111111111'::uuid,
      'Asignación automática de tickets alta prioridad',
      jsonb_build_object('ticket', jsonb_build_object('priority', 'Alta')),
      jsonb_build_object('type', 'assign_least_loaded_agent'),
      true
    )
) as v(department_id, name, trigger_condition, action, is_active)
where not exists (
  select 1
  from public.workflows w
  where w.department_id = v.department_id
    and w.name = v.name
);
...
-- Backfill user-friendly names (solo si no hay valores definidos)
...
-- ---------------------------------------------------------------------
-- Tasks (tareas): acciones operativas por ticket/chat/general
-- ---------------------------------------------------------------------
...
-- Reporting: breakdowns + tops (admin/supervisor)
